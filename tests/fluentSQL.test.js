...
describe('Test suite for FluentSQL buider ', () => {
  ...
  test('#buid should return the empty object instance', () => {
    const result = FluentSQLBuilder.for(data).build()
    const expected = data
    expect(result).toStrictEqual(expected)
  })

  test('#limit given a collection it should limits result', () => {
  ...
  test('#select given a collection it return only specific field', () => {
    const result = FluentSQLBuilder.for(data)
